...
%% Create map
figure
axesm('MapProjection', 'eqaazim', 'flatlimit', [-Inf 13], 'Origin', [p_lat p_lon 0], 'Frame', 'on');
...
%% Overlay L-shells
l_shells = [2 3 4];
[lats_n,lons_n,lats_s,lons_s] = LShell_lines([2001 1 1 0 0 0],l_shells,300,100);
...
station_names = {'Rothera', 'Ferraz', 'Vernadsky'};
station_pos = [-67.57, -68.13;
               -62.13, -58.67;
			   -65.25, -64.26];
colors = {'r', [0 0.8 0], 'b'};

for kk = 1:length(station_names)
	plotm(station_pos(kk,1), station_pos(kk,2), 'o', 'color', colors{kk}, 'markerfacecolor', colors{kk});
	textm(station_pos(kk,1), station_pos(kk,2), [' ' station_names{kk}], 'color', colors{kk}, ...
		'fontweight', 'bold', 'verticalalignment', 'cap');
end
