function varargout = sp_emission_db_characterizer_old(varargin)
...
%      singleton*.
%
%      H = SP_EMISSION_DB_CHARACTERIZER_OLD returns the handle to a new SP_EMISSION_DB_CHARACTERIZER_OLD or the handle to
%      the existing singleton*.
%
%      SP_EMISSION_DB_CHARACTERIZER_OLD('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SP_EMISSION_DB_CHARACTERIZER_OLD.M with the given input arguments.
%
%      SP_EMISSION_DB_CHARACTERIZER_OLD('Property','Value',...) creates a new SP_EMISSION_DB_CHARACTERIZER_OLD or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sp_emission_db_characterizer_old_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sp_emission_db_characterizer_old_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sp_emission_db_characterizer_old

% Last Modified by GUIDE v2.5 27-Apr-2011 08:23:19
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @sp_emission_db_characterizer_old_OpeningFcn, ...
                   'gui_OutputFcn',  @sp_emission_db_characterizer_old_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before sp_emission_db_characterizer_old is made visible.
function sp_emission_db_characterizer_old_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sp_emission_db_characterizer_old (see VARARGIN)

% Choose default command line output for sp_emission_db_characterizer_old
handles.output = hObject;

% UIWAIT makes sp_emission_db_characterizer_old wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% Add empty emissions struct

% Set defaults
[database_filename, cleaned_data_directory, initial_index] = sp_emission_db_characterizer_defaults;
...
function varargout = sp_emission_db_characterizer_old_OutputFcn(hObject, eventdata, handles) 
...
switch type
  ...
  case 'emission'
    set(handles.edit_current_char, 'BackgroundColor', get(handles.pushbutton_chorus_bound, 'BackgroundColor'));
  case 'noise'
    set(handles.edit_current_char, 'BackgroundColor', get(handles.pushbutton_hiss_bound, 'BackgroundColor'));
  otherwise
...
% --- Executes on button press in pushbutton_chorus_bound.
function pushbutton_chorus_bound_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_chorus_bound (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in pushbutton_hiss_bound.
function pushbutton_hiss_bound_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_hiss_bound (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if ~exist(get('hObject', 'string'), 'file')
  uiwait(errordlg(sprintf('File ''%s'' does not exist', get('hObject', 'string')), 'File does not exist'));
end
