function [hour, minute, second] = sunset(lat,lon,alt,civil_sun,year,month,day);

%finds sunrise time
%civil_sun is 1 or 0
%alt is altitude (ignored if civil_sun = 1, used if civil_sun = 0)

%sunset: sun_dark goes from 1 to 0:
...
try
    ...
    sun_dark(find(sun_dark<1)) = 0; %ignore twilight
    if(min(min(sun_dark))==1)  %always dark until very end of minute
        second = 59;
    else  
