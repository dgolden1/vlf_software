function [data_cleaned, data_orig_slowtail] = remove_sferics_test(data_bb, fs_bb, t_imp_start, t_imp_end, b_slowtail_filter, p)
...
bad_start_indices = nearest(t_imp_start, t_data_bb);
...
% difference.
bad_end_indices = nearest(t_imp_end, t_data_bb) - 1;
...
%   idx_estimate = idx_good_before; % Indices to use for estimation
  ...
  [a, g] = lpc(data_cleaned(idx_good_before), p);
...
%   blah_before = idx_good_before(end - p:end);
%   x = filter(1, -a(2:end), [data_bb(blah_before); zeros(size(idx_bad.'))]);
%   plot([blah_before, idx_bad], [data_bb(blah_before); zeros(size(idx_bad.'))], ...
%     [blah_before, idx_bad], x);
% 
%   su_alt = zeros(size(su));
%   data_before = data_bb(idx_good_before);
%   for jj = 1:length(su_alt)
%     idx_clean = length(data_before):-1:length(data_before) - p + jj;
%     idx_a_clean = jj + 1:p + 1;
%     idx_unclean = jj - 1:-1:max(1, jj - p);
%     idx_a_unclean = 2:min(jj, p+1);
%     
%     su_alt(jj) = sum(-data_before(idx_clean).*a(idx_a_clean).') + ...
%       sum(-su_alt(idx_unclean).*a(idx_a_unclean).');
%     
%     plot(idx_good_before, data_before, idx_bad, su_alt);
%   end
end
