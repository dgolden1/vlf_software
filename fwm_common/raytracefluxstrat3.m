function [r0,n0,n20,tau,beamlen,dAdOmega,SS0,rp,np]=...
	raytracefluxstrat2(thp0,php0,thB,h,wp2,w,wH,mode,alfa,debugg)
...
for it=1:nmax-1
	...
	if it/2==round(it/2)
        ...
        %sqrt(n2a(it,:).*(1+tana.^2))
        %dnp(3,:)
        change=max([1 sqrt(n2a(it,:).*(1+tana.^2)) abs(dnp(3,:))./sqrt(n2a(it,:))/alfa]);
        dtau=dh/change; % Adjust the path step
