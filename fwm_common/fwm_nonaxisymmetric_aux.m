function result=fwm_nonaxisymmetric_aux(np_arg,phitry,zd,eground,perm,...
...
if point_source
    ...
    switch coornperp
		case 1
			...
			I0ep=permute(I0e,[4 3 1 2]);
			I0ei=zeros(3,Ms,Nnp); % Initialize
		case 2
			error('not impemented');
		case 3
			% To size Nnp0 x 3 x Ms x Nh and back to 3 x Ms x Nnp x Nh
			I0ep=permute(interp1(np0,permute(I0e,[3 1 2 4]),np_arg),[2 3 1 4]);
		otherwise
			error('unknown coornperp');
	end
end
...
for iphitry=1:Nphitry
    ...
    EHf(:,:,:,iphitry)=fwm_field(zd,eground,perm,...
        ksa,nx,ny,Ie0i,Im0i,EHfu0i,EHfd0i,kia,dzl,dzh);
    timec=now*24*3600;
    ttot=timec-tstart;
    if do_output
