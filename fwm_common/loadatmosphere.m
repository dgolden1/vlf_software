...
for iprofile=1:nprofiles
    ...
    disp(['loading ' name]);
    iri=load(fullfile(global_dir, ['densities/IRI_' name '.txt']));
    ...
    if exist(fullfile(global_dir, ['densities/MSIS1_' name '.txt']))==2
        a=load(fullfile(global_dir, ['densities/MSIS1_' name '.txt']));
        b=load(fullfile(global_dir, ['densities/MSIS2_' name '.txt']));
...
for k=1:length(su_names)
    name=su_names{k};
    disp(['loading ' name]);
    eprof=load(fullfile(global_dir, ['densities/' name '.dat']));
    IONOSPH={IONOSPH{:},...
