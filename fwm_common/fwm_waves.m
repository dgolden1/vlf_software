function varargout=fwm_waves(varargin)
...
arginnames={'zd','eground','perm','dn0','npb','np',...
    'ksa','coornperp','nperp1','nperp2','I0','kia','dzl','dzh'};
...
if point_source
	...
	switch coornperp
		...
		case 3
			% Interpolate at np
			% To size Nnp0 x 6 x Ms x Nh and back to 6 x Ms x Nnp x Nh
			I0p=permute(interp1(np0,permute(I0,[3 1 2 4]),np),[2 3 1 4]);
		otherwise
			error('unknown coornperp');
	end
	I0i=zeros(6,Ms,Nphimax); % pre-allocate
end
    ...
    if ~point_source
		switch coornperp
			...
			case 3
				...
				% Size = 6 x Ms x Nphi(ip) x Nh
				e=permute(repmat(exp(i*m(:)*p(:).'),[1 1 6 Ms]),[3 4 2 1]);
				I0i(:,:,1:Nphi(ip))=sum(repmat(I0p(:,:,ip,:),[1 1 Nphi(ip) 1]).*e,4);
			otherwise
				error('unknown coornperp');
		end
	end
    EHf(:,:,ii)=fwm_field(zd,eground,perm,ksa,nxtmp,nytmp,I0i(:,:,1:Nphi(ip)),kia,dzl,dzh);
