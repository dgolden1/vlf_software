function [EH0,EHf0,np,ki2,npt,EHft0]=fwm_axisymmetric(f,h,eground,perm,...
	ksa,np0,m,Ie0,Im0,EHfu0,EHfd0,rkm,hi,rmaxkm,drkm,retol)
...
% Usage:
%    EH=impedance0*fwm_axisymmetric(f,h,eground,perm,...
%       ksa,np0,m,Ie0,[],[],[],rkm,hi[,rmaxkm,drkm,retol])
% Advanced usage:
...
%    ksa, np0, m, I0e, I0m, EHf0u, EHf0d - the sources
...
%       Ie0 (3 x Ms x N0) - the value of the current moments or surface
%          currents at altitudes h(ksa). Given are the current component
%          values on the positive x-axis (at phi==0) (see note 3!)
%       (optional: I0m, EHf0u, EHf0d)
%       Im0 (3 x Ms x N0) - the magnetic current moments
%       EHf{u|d}0 (4 x N0) - initial horizontal field just above the
%          {lower|upper} boundary (note that only {upward|downward} modes
...
% Add the incoming waves as source
if ~isempty(EHfu0)
    zds=[zd(1);zds];
end
if ~isempty(EHfd0)
...
[npb,np,EHf0,relerror]=bestgrid(@fwm_axisymmetric_aux,...
    {zd,eground,perm,ksa,np0,Ie0,Im0,EHfu0,EHfd0,kia,dzl,dzh},...
    npbi,dn0,retol);
...
[npeb,npe,EHfe0,relerror]=bestgrid(@fwm_axisymmetric_aux,...
    {zde,eground,perme,ksa,np0,Ie0,Im0,EHfu0,EHfd0,kiavac,dzlvac,dzhvac},...
    npebi,dn0,retol);
