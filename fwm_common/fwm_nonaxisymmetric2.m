function [EH,EHf,nx,ny,da,np,Nphi]=fwm_nonaxisymmetric2(...
    f,h,eground,perm,...
    ksa,coornperp,nperp1,nperp2,...     % Input: the current/field
    Ie0,Im0,EHfu0,EHfd0,...             % Input: the current/field
    coorrperp,rperp1,rperp2,hi,...      % Output: the spatial coordinates
    rmaxkm,drkm,retol,bkp_file)
%FWM_NONAXISYMMETRIC2 Full-wave method, the general case
% Usage:
%    EH=impedance0*fwm_nonaxisymmetric(f,h,eground,perm,...
...
%       I0 (3 x Ms x {Nnx0|Nnp0|Nnp0} x {Nny0|Nphin0|Nh0}) - the value of
...
if do_backups
	...
	args={f,h,perm,eground,...
		ksa,coornperp,nperp1,nperp2,Ie0,Im0,EHfu0,EHfd0,hi,rmaxkm,retol};
...
if do_backups & restore
	...
	[npb,np,EHftry,relerror]=bestgrid(@fwm_nonaxisymmetric_aux,...
        {[0],zd,eground,perm,...
...
[EHf,nx,ny,da,Nphi]=fwm_waves2(zd,eground,perm,dn0,npb,np,...
	ksa,coornperp,nperp1,nperp2,Ie0,Im0,EHfu0,EHfd0,kia,dzl,dzh,bkp_file);
...
if point_source
	...
	I0p=permute(I0h(:,:,:,2),[3 1 2]); % (Nnp0 x 3 x Ms)
